...
type TaskLogPO struct {
	...
	Name     string                                 `gorm:"size:64;not null;comment:任务组名称;index:idx_name_logLevel,priority:1;index:idx_name,priority:1"`
	...
	LogLevel eumLogLevel.Enum                       `gorm:"type:tinyint;not null;comment:日志级别;index:idx_name_logLevel,priority:2"`
	Content  string                                 `gorm:"type:text;size:0;not null;comment:日志内容"`
	CreateAt time.Time                              `gorm:"type:timestamp;size:6;not null;comment:日志时间;index:idx_name_logLevel,priority:3;index:idx_name,priority:2"`
}
